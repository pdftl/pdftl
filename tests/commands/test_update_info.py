...
def test_update_info_stdin_flag():
    ...
    with patch("pdftl.commands.update_info.resolve_operation_spec") as mock_resolve:
        mock_resolve.return_value = PdfInfo()  # Return a dummy result
        with patch("pdftl.commands.update_info.execute_update_info"):
...
def test_update_info_json_file_legacy_decoding(tmp_path):
    ...
    with patch("pdftl.commands.update_info.execute_update_info") as mock_exec:
