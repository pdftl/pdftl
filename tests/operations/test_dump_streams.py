...
def test_collect_page_stream_normalization_disabled(base_pdf):
    ...
    # 2. Page with multi-stream array
    ...
    # 3. Page with single content stream (Covers line 230)
...
def test_shared_xobjects_warning_output(base_pdf):
    ...
    warnings = [w for header, _, w in res.data if "Page 1 / XObject" in header][0]
...
def test_recurse_and_collect_comprehensive(base_pdf):
    ...
    # Validate collected payloads (should extract Nested XObject, Nested resources, and Empty XObject)
    assert len(collected) == 3
    empty_warnings = [w for h, c, w in collected if "FmEmpty" in h][0]
