...
- `excise` outputs cleaner content streams
...
- Bump minimum `pikepdf` version to fix `AttributeError` when reading page rotation during `cat` operations.

## [0.28.0] - 2026-08-30

### Added

- `prune_resources` output option

- `deduplicate_icc_profiles` operation

- `deduplicate_fonts` operation

- `deduplicate_images` operation
...
- A warning is issued when saving will invalidate a signature

### Fixed

- `excise` and `redact` now remove annotations whose rectangle cannot be determined

- `replace`, `style_text`, `mutate_content`, `simplify_vectors`,
  `recolor_vectors`, `dump_streams` and `import_streams`
  now also recurse into tiling Patterns, ExtGState
...
- add automatic bookmark detection to `update_bookmarks`
